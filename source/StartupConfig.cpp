...
StartupConfig StartupConfig::load(const String& filename) {
	...
	if (!FileSystem::exists(filename)) {
		config.toAny(true).save(filename, config.jsonOutput);		// If the file doesn't exist create it
	}
...
StartupConfig::StartupConfig(const Any& any) {
	...
	case 1:
		...
		reader.getIfPresent("jsonOutput", jsonOutput);
...
Any StartupConfig::toAny(const bool forceAll) const {
	...
	if (forceAll || def.jsonOutput != jsonOutput)									a["jsonOutput"] = jsonOutput;
...
bool StartupConfig::validateExperiments() const {
	...
	for (auto& configs : experimentList) {
		ExperimentConfig experimentConfig = ExperimentConfig::load(configs.experimentConfigFilename, jsonOutput);
