#!/bin/bash
# Autogenerated 2020 06 26 12:42:19
mkdir -p dist/sound/
...
cp c:/g3d/G3D10/data-files/shader/UniversalSurface/UniversalSurface_depthPeel.pix dist/shader/UniversalSurface/
cp c:/g3d/G3D10/data-files/shader/DefaultRenderer/DefaultRenderer_OIT_writePixel.glsl dist/shader/DefaultRenderer/
cp `which tbbmalloc.dll` dist/
cp `which tbb.dll` dist/
cp `which swscale-5.dll` dist/
cp `which swresample-3.dll` dist/
cp `which postproc-55.dll` dist/
cp `which optix.6.0.0.dll` dist/
cp `which fmod64.dll` dist/
cp `which embree.dll` dist/
cp `which cudart64_102.dll` dist/
cp `which avutil-56.dll` dist/
cp `which avformat-58.dll` dist/
cp `which avfilter-7.dll` dist/
cp `which avdevice-58.dll` dist/
cp `which avcodec-58.dll` dist/
cp README.txt dist/
cp LICENSE.txt dist/
cp Build/FirstPersonScience-x64-Release/FirstPersonScience.exe dist/
