...
public:
	...
	ExperimentConfig(const Any& any) {
		...
		case 1:
            ...
            reader.get("reactions", reactions);
	...
	shared_ptr<ReactionConfig> getReactionConfigById(String id) {
		for (int i = 0; i < reactions.size(); i++) {
            if (!reactions[i].id.compare(id)) {
                return std::make_shared<ReactionConfig>(reactions[i]);
            }
		}
		return nullptr;
	}
	...
	Array<Param> getReactionExpConditions(int sessionIndex) {
		Array<Param> params;
		for (int j = 0; j < sessions[sessionIndex].trials.size(); j++) {
            shared_ptr<ReactionConfig> rc = getReactionConfigById(sessions[sessionIndex].trials[j].id);
			for (float intensity : rc->intensities) {
				Param p;
